...
export default function SignupForm({flip}) {
  ...
  const onSubmit = async (data) => {
    ...
    alert("Form submitted"); // Test if this appears

    setIsLoading(true);
    try {
      const response = await axios.post('http://localhost:8080/api/v1/user/new', {
      ...
      toast({
        ...
        description: error.response?.data?.message || "Invalid request. Please check your input and try again.",
